...
//*****************************************************************************************
   ...
   Tim::CSVReader reader("/home/pranka/work/Strat_Init/Strat/temp/open_pos_bnf_.csv");
   csv_pos = reader.getData();
    // for(int i = 0; i < csv_pos.size(); i++){
        // for(int j = 0; j < df[i].size(); j++){
        // std::string hr24_str = df[i][0];
        // struct tm t2 = {0};
        // t2.tm_year = Tim::Helpers::getYear(configReader.date);
        // t2.tm_mon = Tim::Helpers::getMonth(configReader.date);
        // t2.tm_mday = Tim::Helpers::getDay(configReader.date);
        // t2.tm_hour = std::stoi(hr24_str)/100;
        // t2.tm_min = std::stoi(hr24_str) % 100;;
        // t2.tm_sec = 0;
        // std::time_t tSE = mktime(&t2);
        // uint64_t coltime = long(timeSinceEpoch)*E9;
