...
print("Good here", start_date, end_date, sym)
sys.stdout.flush()
...
def excute(dates):
    ...
    while True:
        ...
        cmd = []
        for i in date_range:
            cmd.append(Popen('~/bin/getAxisSym_Futures.sh ' + str(i) + " " + sym, shell=True))
        for c in cmd:
            c.wait()
        #for i in date_range:
        #    with open("/home/hpuria/dated_symbols/BNFTO/" + str(i) + "/symbols", "r") as f:
        #        lines = f.readlines()
        #    with open("/home/hpuria/dated_symbols/BNFTO/" + str(i) + "/symbols", "w") as f:
        #        for line in lines:
        #            ll = line
        #            symb = line.split(" ")[1]
        #            print(symb)
