...
def excute(dates, typ):
    ...
    while True:
        ...
        cmd = []
        for i in date_range:
            cmd.append(Popen('~/bin/getAxisBankniftySym.sh ' + str(i) + " " + str(strike_range[0]) + " " + str(
                strike_range[1]) + " " + sym + " " + typ, shell=True))
        for c in cmd:
            c.wait()
        for i in date_range:
            with open("/home/pranka/dated_symbols_banknifty/" + str(i) + "/symbols", "r") as f:
                lines = f.readlines()
            with open("/home/pranka/dated_symbols_banknifty/" + str(i) + "/symbols", "w") as f:
                for line in lines:
                    ll = line
                    symb = line.split(" ")[1]
                    print(symb)
