...
void createGraph(int date){
    ...
    dataDumper = new Baryons::DataDumper(date, sample_starttime, freq * SECOND,unHand,
		params, dump_params,&spot_obj_map, strat_params->delta_lower_cap, strat_params->delta_upper_cap, modelHandler);
    ...
    greekTimerHand->addGreekTimerListener(rgTimerHand);
    rgTimerHand->addGreekTimerListener(dataDumper);
    // greekTimerHand->addGreekTimerListener(strategyTimerHand);
...
static void md_cb(uint16_t symbolID, const illuminati::md::MarketUpdateNew* md){
    ...
    // if (md->m_symbol == "NIFTY     "){
        // std::cout << "Printing stuff " << md->m_symbol << ",  " << "NewPrice: " << md->m_newPrice << std::endl;
        // return; 
    // } 
    std::cout << "Printing stuff " << md->m_symbol << ",  " << "NewPrice: " << md->m_newPrice << std::endl;
...
int main(int argc, char* argv[]){
    ...
    for(auto x = mySyms.begin(); x != mySyms.end(); x++){
        for (auto y = x->second.begin(); y != x->second.end(); y++){
            ...
            std::cout << traded_ticker_list[i] << std::endl;
            i++;
        }
    ...
    MyLogger::log(TRADEFILE, "main::init |", "Trade Universe initialized\n");
    // ticker_list += "NIFTY     ,";
    // ticker10len_list+= "NIFTY     ,";
    // ticker_vec.push_back("NIFTY     ");
    // traded_ticker_list.push_back("NIFTY     ");
    ...
    con = new ISHM::NSEConnectorWrapper(&md_cb, &resp_cb, myCon.connectorCfg.INTERACTION_MODE,
        &(myCon.connectorCfg), ticker_vec); //isRaw? ticker_vec :v);
    // ticker_list+=",NIFTY     ";
    ...
    for(auto it=unHand->TradeUniverseIDs.begin(); it!=unHand->TradeUniverseIDs.end(); ++it) {
        std::cout << it->first << ": " << it->second->expiry_name << "\n";
    }
...
//   unHand->IVCalendarUniverse("Week2",Tim::Week2);
