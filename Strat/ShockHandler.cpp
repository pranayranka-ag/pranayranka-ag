...
namespace Baryons{
    ...
    filepath(filepath_t)
  {
    ...
    all_moves.push_back(std::make_pair(0.0,0.0));
    for (int j = 0; j < iv_moves.size(); j++) {
    	all_moves.push_back(std::make_pair(0.0,iv_moves[j]));
    }
    for (int i = 0; i < spot_moves.size(); i++) {
			all_moves.push_back(std::make_pair(spot_moves[i],0.0));
    }

	}
  
  void ShockHandler::findMarginShock(uint64_t timestamp){
	  std::vector<double> a(int(all_moves.size()),0);
	  ...
	  if (logger_i2==0) {
		...
		++logger_i2;
		out+="spot_0_iv_0,spot_0_iv_10,spot_0_iv_15,spot_0_iv_20,spot_0_iv_-10,spot_0_iv_-15,spot_0_iv_-20,spot_2_iv_0,spot_5_iv_0,spot_10_iv_0,spot_15_iv_0,spot_-2_iv_0,spot_-5_iv_0,spot_-10_iv_0,spot_-15_iv_0,";
		out.pop_back();
		out+="\n";
	  ...
	  for(int z = 0; z < all_moves.size(); z++){
		ss << a[z] << ",";
		a[z] = 0;
	  }
	  std::string ss_str = "";
